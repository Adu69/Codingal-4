...
label1 = Label(root, 
               text = 'Hey user! Welcome to Denomination Counter Aplication.',
               bg = 'light blue',)
label1.place(relx = 0.5, y = 340, anchor = CENTER)

def msg():
    MsgBox = messagebox.showinfo(
        "Alert", "Do you want to calculate the denomination amount?")
    if MsgBox == 'ok':
        topwin()
...
def topwin():
    ...
    entry = Entry(top)
    lbl = Label(top, text = 'Here are the number of note for each denomination', bg = 'light grey')
